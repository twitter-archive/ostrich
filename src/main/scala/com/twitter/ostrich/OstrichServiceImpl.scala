...
class OstrichServiceImpl(config: OstrichServiceConfig) extends OstrichServiceServer {
  val serverName = "Ostrich"
  ...
  def get(key: String) = {
    database.get(key) match {
      case None =>
        ...
        Future.exception(new OstrichException("No such key"))
      case Some(value) =>
