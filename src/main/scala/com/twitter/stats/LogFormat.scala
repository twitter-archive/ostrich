...
/**
 * A format for log lines of stats.
 *
 * Note that instances may track state over multiple invocations (e.g., for formats that require
 * line counting or content hashes).
 */
trait LogFormat {
  /**
   * Generates a header line for the specified keys. Override if your log format has header lines.
   * Ignore otherwise.
   */
